...
视频对标的目标不是评价用户自己的素材，而是让用户上传一个想学习的参考视频，由 Mimo v2.5 以专业视频分析人员的角度拆解参考视频，并结合用户自己的 IP、产品、定位和目标客户，输出可执行的模仿和翻拍方案。
...
- `benchmark`：显示 `对标 / 平台 / 目标客户`，无平台时显示 `视频对标`。
